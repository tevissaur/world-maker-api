...
connection_1.default.once('open', async () => {
    await models_1.Character.deleteMany({});
    await models_1.City.deleteMany({});
    await models_1.Country.deleteMany({});
    await models_1.God.deleteMany({});
    await models_1.Race.deleteMany({});
    await models_1.Region.deleteMany({});
    await models_1.Religion.deleteMany({});
    await models_1.User.deleteMany({});
    await models_1.World.deleteMany({});
    const characters = await models_1.Character.insertMany(characterData);
    const cities = await models_1.City.insertMany(cityData);
    const countries = await models_1.Country.insertMany(countryData);
    // const classes = await Class.insertMany(classData)
    const gods = await models_1.God.insertMany(godsData);
    ...
    const races = await models_1.Race.insertMany(raceData);
    const regions = await models_1.Region.insertMany(regionData);
    const religions = await models_1.Religion.insertMany(religionData);
    const users = await models_1.User.create(userData);
    // const users = await User.insertMany(userData)
    const worlds = await models_1.World.insertMany(worldData);
